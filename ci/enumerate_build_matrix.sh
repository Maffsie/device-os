...
DEBUG_BUILD=( y n )
PLATFORM=( photon p1 electron argon boron xenon asom bsom b5som )
# P1 bootloader built with gcc 4.8.4 doesn't fit flash, disabling for now
PLATFORM_BOOTLOADER=( photon electron argon boron xenon asom bsom b5som tracker )
APP=( "" tinker product_id_and_version)
TEST=( wiring/api wiring/no_fixture wiring/no_fixture_long_running )

MODULAR_PLATFORM=( photon p1 electron argon boron xenon asom bsom b5som tracker )
...
do
  ...
  do
    ...
    if [[ "$db" = "y" ]]; then
      if [[ "$p" = "photon" ]] || [[ "$p" = "p1" ]] || [[ "$p" = "electron" ]] || [[ "$p" = "argon" ]] || [[ "$p" = "boron" ]] || [[ "$p" = "xenon" ]] || [[ "$p" = "asom" ]] || [[ "$p" = "bsom" ]] || [[ "$p" = "b5som" ]] || [[ "$p" = "tracker" ]]; then
        continue
      fi
    fi
    cmd="${MAKE} DEBUG_BUILD=\"$db\" PLATFORM=\"$p\" COMPILE_LTO=\"n\""
